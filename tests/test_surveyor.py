...
def test_survey_cbc(runner, mocker):
    ...
    mocked_func = mocker.patch(
        "products.vmware_cb_enterprise_edr.CbEnterpriseEdr._authenticate"
    )
...
def test_survey_dfe(runner, mocker):
    ...
    """
    mocked_func = mocker.patch(
        "products.microsoft_defender_for_endpoints.DefenderForEndpoints._authenticate"
    )
    with runner.isolated_filesystem() as temp_dir:
        cred_file_path = os.path.join(temp_dir, "test_creds.ini")
        with open(cred_file_path, "w") as deffile:
...
def test_custom_query(runner, mocker):
    ...
    mocked_process_search = mocker.patch(
        "products.vmware_cb_response.CbResponse.process_search"
    )
    ...
    mocked_process_search.assert_called_once_with(
        Tag("query"), {}, "SELECT * FROM processes"
    )
...
def test_def_file(runner, mocker):
    ...
    """
    mocker.patch("products.vmware_cb_response.CbResponse._authenticate")
    mocked_nested_process_search = mocker.patch(
        "products.vmware_cb_response.CbResponse.nested_process_search"
    )
    with runner.isolated_filesystem() as temp_dir:
        ...
        mocked_nested_process_search.assert_called_once_with(
            Tag("ProgramA", "test_deffile"), {"process_name": ["test.exe"]}, {}
        )
...
def test_def_dir(runner, mocker):
    ...
    with runner.isolated_filesystem() as temp_dir:
        ...
        expected_calls = [
            mocker.call(
                Tag("ProgramA", "test_deffile1"), {"process_name": ["test1.exe"]}, {}
            ),
            mocker.call(
                Tag("ProgramB", "test_deffile2"), {"process_name": ["test2.exe"]}, {}
            ),
        ]
...
def test_ioc_file(runner, mocker):
    ...
    with runner.isolated_filesystem() as temp_dir:
        ...
        mocked_func.assert_called_once()
        mocked_nested_process_search.assert_called_once_with(
            Tag(f"IOC - {ioc_file_path}", "ioc_list.txt"), {"ipaddr": ["127.0.0.1"]}, {}
        )
...
def test_ioc_file_with_base_query(runner, mocker):
    ...
    with runner.isolated_filesystem() as temp_dir:
        ...
        result = runner.invoke(
            cli, ["--iocfile", ioc_file_path, "--ioctype", "ipaddr"] + filter_args
        )
        assert "Processing IOC file" in result.output
        mocked_func.assert_called_once()
        mocked_nested_process_search.assert_called_once_with(
...
def test_unsupported_command(runner):
    ...
    assert "Error: No such command 'asdfasdfasdfasdf'" in result.output
    assert result.exit_code != 0


def test_unsupported_option(runner):
    result = runner.invoke(cli, ["--asdfasdfasdfasdf"])
    assert "Error: No such option: --asdfasdfasdfasdf" in result.output
    assert result.exit_code != 0
...
def test_mutually_exclusive_output_prefix(runner, mocker):
    mocker.patch("products.vmware_cb_response.CbResponse._authenticate")
    result = runner.invoke(
        cli, ["--prefix", "test_prefix", "--output", "test_output.csv"]
    )
...
fields:
    ...
    - ParentCommandLine""")
        ...
        expected_calls = [
            mocker.call(
                Tag(
                    "Test sigma rule - 5fd18e43-749c-4bae-93b6-d46e1f27062e",
                    "Sigma Rule",
                ),
                {"query": ["process_name:curl.exe"]},
                {},
            ...
            mocker.call(
                Tag(
                    "Test sigma rule 2 - 15ecb82d-b7c0-4e53-9bf3-deedb4c9908c",
                    "Sigma Rule",
                ),
