...
def test_build_query_with_supported_field(cbr_product: CbResponse):
    ...
    assert (
        result
        == ' hostname:workstation1 username:admin start:-1440m start:-10m (group:"accounting dept")'
    )
...
def test_process_search(cbr_product: CbResponse, mocker):
    ...
    cbr_product._conn.select.return_value.where.assert_called_once_with(
...
def test_nested_process_search(cbr_product: CbResponse, mocker):
    with open(
        os.path.join(os.getcwd(), "tests", "data", "cbr_surveyor_testing.json")
    ) as f:
    ...
    cbr_product._conn.select.return_value.where.assert_has_calls(
...
def test_nested_process_search_limit_option(cbr_product: CbResponse, mocker):
    ...
    cbr_product._conn.select = mocker.Mock()
    cbr_product._conn.select.return_value = mocker.Mock(where=mocked_query_return)
    cbr_product.nested_process_search(
        Tag("test_tag"), {"query": "process_name:cmd.exe"}, {}
    )
...
def mocked_query_return(query: str):
    return [
        MockProcResult(
            "workstation1", "username1", "path1", "cmdline1", "start1", "id1"
        ),
        MockProcResult(
            "workstation2", "username2", "path2", "cmdline2", "start2", "id2"
        ),
        MockProcResult(
            "workstation3", "username3", "path3", "cmdline3", "start3", "id3"
        ),
    ]
