...
class MockProcResult:
    def get_details(self):
        ...
        return {
            "device_name": f"workstation{random_num}",
            "process_username": [f"username{random_num}"],
            "process_name": f"proc{random_num}",
            "process_cmdline": [f"cmdline{random_num}"],
            "device_timestamp": f"ts{random_num}",
            "process_guid": f"guid{random_num}",
        }
...
def test_build_query_with_supported_field(cbc_product: CbEnterpriseEdr):
    ...
    assert (
        result
        == 'device_name:workstation1 process_username:admin (device_group:"accounting dept") (device_policy:"strict")'
    )
...
def test_build_query_with_days(cbc_product: CbEnterpriseEdr):
    ...
    assert datetime.strptime(timespan[1], time_format) - timedelta(
        days=7
    ) == datetime.strptime(timespan[0], time_format)
...
def test_process_search(cbc_product: CbEnterpriseEdr, mocker):
    ...
    cbc_product.perform_query.assert_called_once_with(
        Tag("test_field"), {}, "process_name:cmd.exe"
    )


def test_nested_process_search(cbc_product: CbEnterpriseEdr, mocker):
    with open(
        os.path.join(os.getcwd(), "tests", "data", "cbc_surveyor_testing.json")
    ) as f:
    ...
    expected_calls = [
        ...
        mocker.call(
            Tag("field_translation"), {}, "(netconn_domain:raw.githubusercontent.com)"
        ),
        ...
        mocker.call(
            Tag("multiple_values"),
            {},
            "(process_name:svchost.exe OR process_name:cmd.exe)",
        ),
        mocker.call(Tag("single_query"), {}, "(process_name:rundll.exe)"),
        mocker.call(
            Tag("multiple_query"),
            {},
            "((process_cmdline:-enc) OR (modload_name:malware.dll))",
        ),
    ]
...
def test_perform_query_with_limit_option(cbc_product: CbEnterpriseEdr, mocker):
    ...
    assert (
        len(cbc_product.perform_query(Tag("test_tag"), {}, "process_name:pwsh.exe"))
        == 2
    )
...
def mocked_query_return(full_query: str):
    return [MockProcResult(), MockProcResult(), MockProcResult()]
