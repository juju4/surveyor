...
def test_build_query_with_supported_field(cortex_product: CortexXDR):
    ...
    assert (
        result
        == ' | filter agent_hostname contains "workstation1" | filter action_process_username contains "admin" or actor_primary_username contains "admin"'
    )
...
def test_build_query_with_unsupported_field(cortex_product: CortexXDR):
    filters = {"useless key": "asdfasdasdf"}
...
def test_process_search(cortex_product: CortexXDR):
    ...
    assert (
        cortex_product._queries[Tag("test_query")][0].relative_time_ms
        == 14 * 24 * 60 * 60 * 1000
    )
...
def test_nested_process_search(cortex_product: CortexXDR):
    ...
    with open(
        os.path.join(os.getcwd(), "tests", "data", "cortex_surveyor_testing.json")
    ) as f:
    ...
    assert (
        Query(relative_ts, "action_process_image_name", "contains", '"cmd.exe"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_remote_ip", "contains", '"8.8.8.8"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_process_command_line", "contains", '"grep"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(
    ...
    assert (
        Query(relative_ts, "action_module_path", "contains", '"asdf.dll"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_file_path", "contains", '"helloworld.txt"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_registry_key_name", "contains", '"HKCU"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_process_image_md5", "contains", '"asdfasdfasdf"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_process_image_sha256", "contains", '"qwerqwerqwer"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_remote_port", "contains", '"80"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_file_md5", "contains", '"zxcvzxcvzxcv"')
        in cortex_product._queries[Tag("field_translation")]
    )
    assert (
        Query(relative_ts, "action_file_sha256", "contains", '"poiupoiupoiu"')
        in cortex_product._queries[Tag("field_translation")]
    )
    ...
    assert (
        ...
        in cortex_product._queries[Tag("multiple_values")]
    )

    assert len(cortex_product._queries[Tag("single_query")]) == 1
    assert (
        Query(relative_ts, None, None, None, "FieldA=ValueB")
        in cortex_product._queries[Tag("single_query")]
    )

    assert len(cortex_product._queries[Tag("multiple_query")]) == 2
    assert (
        Query(relative_ts, None, None, None, "FieldA=ValueB")
        in cortex_product._queries[Tag("multiple_query")]
    )
    assert (
        Query(relative_ts, None, None, None, "FieldC=ValueD")
        in cortex_product._queries[Tag("multiple_query")]
    )
...
def test_process_queries_full_query(cortex_product: CortexXDR, mocker):
    ...
    mocked_func = mocker.patch.object(
        cortex_product._session, "post", return_value=response_mock
    )
    ...
    mocked_func.assert_called_once_with(
        "https://cortex.xdr.domain/public_api/v1/xql/start_xql_query/",
        headers={},
