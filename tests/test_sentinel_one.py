...
def test_init_dv_lower_limit_option(tmpdir, mocker):
    ...
    s1_product = SentinelOne(
        profile="default",
        creds_file=cred_file_path,
        account_id=None,
        site_id=None,
        account_name=None,
        pq=False,
...
def test_build_query_with_supported_field_dv(s1_product: SentinelOne):
    filters = {"hostname": "workstation1", "username": "admin"}
    s1_product._pq = False
    base, from_date, to_date = s1_product.build_query(filters)

    assert (
        base
        == 'EndpointName containscis "workstation1" AND UserName containscis "admin"'
    )
...
def test_build_query_with_supported_field_pq(s1_product: SentinelOne):
    filters = {"hostname": "workstation2", "username": "admin1"}
    s1_product._pq = True
    ...
    assert (
        base
        == 'endpoint.name contains "workstation2" and src.process.user contains "admin1"'
    )


def test_build_query_unsupported_keys(s1_product: SentinelOne):
    filters = {"useless key": "asdfasdfasdf"}
    s1_product._pq = False
...
def test_process_search(s1_product: SentinelOne):
    ...
    s1_product.process_search(
        Tag("test_query"), {}, 'FileName containsCIS "svchost.exe"'
    )
    ...
    assert (
        s1_product._queries[Tag("test_query")][0].full_query
        == 'FileName containsCIS "svchost.exe"'
    )
    assert (
        s1_product._queries[Tag("test_query")][0].end_date - timedelta(days=14)
        == s1_product._queries[Tag("test_query")][0].start_date
    )


def test_nested_process_search_dv(s1_product: SentinelOne):
    with open(
        os.path.join(os.getcwd(), "tests", "data", "s1_surveyor_testing.json")
    ) as f:
    ...
    assert (
        Query(sdate, edate, "ProcessName", "containscis", '"notepad.exe"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "IP", "containscis", '"127.0.0.1"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "CmdLine", "containscis", '"MiniDump"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "Publisher", "containscis", '"Microsoft Publisher"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "DNS", "containscis", '"raw.githubusercontent.com"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "TgtFileInternalName", "containscis", '"powershell"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "Url", "containscis", '"https://google.com"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "FilePath", "containscis", '"current_date.txt"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "ModulePath", "containscis", '"pcwutl.dll"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(
            sdate, edate, "SrcProcDisplayName", "containscis", '"Evil Stuff Here"', None
        )
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "Md5", "containscis", '"asdfasdfasdfasdf"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "Sha1", "containscis", '"qwerqwerqwerqwer"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "Sha256", "containscis", '"zxcvzxcvzxcv"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "DstPort", "containscis", '"80"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "RegistryKeyPath", "containscis", '"HKLM"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "RegistryValue", "containscis", '"HKLM"', None)
        in s1_product._queries[Tag("field_translation")]
    )
    ...
    assert (
        ...
        in s1_product._queries[Tag("multiple_values")]
    )
    ...
    assert (
        Query(sdate, edate, "query", "raw", 'FileName containscis "rundll.exe"', None)
        in s1_product._queries[Tag("single_query")]
    )
    ...
    assert (
        Query(
            ...
            "query",
            "raw",
            '(ProcessCmdLine contains "-enc") OR (ModulePath contains "malware.dll")',
            None,
        )
        in s1_product._queries[Tag("multiple_query")]
    )
...
def test_nested_process_search_dv_over_100_terms(s1_product: SentinelOne):
    ...
    s1_product.nested_process_search(
        Tag("list_of_105_terms"), {"process_name": list_o_terms}, {}
    )
    ...
    assert (
        Query(sdate, edate, "ProcessName", "in contains anycase", first_list, None)
        in s1_product._queries[Tag("list_of_105_terms")]
    )
    assert (
        Query(sdate, edate, "ProcessName", "in contains anycase", second_list, None)
        in s1_product._queries[Tag("list_of_105_terms")]
    )
...
def test_nested_process_search_pq(s1_product: SentinelOne):
    ...
    edate = s1_product._queries[Tag("field_translation")][0].end_date
    assert (
        Query(sdate, edate, "src.process.name", "contains", '("notepad.exe")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "dst.ip.address", "contains", '("127.0.0.1")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "src.process.cmdline", "contains", '("MiniDump")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(
    ...
    assert (
        Query(sdate, edate, "tgt.file.internalName", "contains", '("powershell")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "url.address", "contains", '("https://google.com")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "tgt.file.path", "contains", '("current_date.txt")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "module.path", "contains", '("pcwutl.dll")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(
    ...
    assert (
        Query(sdate, edate, "tgt.file.md5", "contains", '("asdfasdfasdfasdf")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "tgt.file.sha256", "contains", '("zxcvzxcvzxcv")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "tgt.file.sha1", "contains", '("qwerqwerqwerqwer")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "module.md5", "contains", '("asdfasdfasdfasdf")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "module.sha1", "contains", '("qwerqwerqwerqwer")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "registry.keyPath", "contains", '("HKLM")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "registry.value", "contains", '("HKLM")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    assert (
        Query(sdate, edate, "dst.port.number", "contains", '("80")', None)
        in s1_product._queries[Tag("field_translation")]
    )
    ...
    edate = s1_product._queries[Tag("single_query")][0].end_date
    assert (
        Query(sdate, edate, None, None, None, 'FileName containscis "rundll.exe"')
        in s1_product._queries[Tag("single_query")]
    )
...
def test_nested_process_search_pq_over_max_char_limit(s1_product: SentinelOne):
    ...
    s1_product.nested_process_search(
        Tag("many_chars"), {"process_name": list_o_terms}, {}
    )
    ...
    assert s1_product._queries[Tag("many_chars")] == [
        Query(sdate, edate, "src.process.name", "contains", first_list, None),
        Query(sdate, edate, "src.process.name", "contains", first_list, None),
        Query(sdate, edate, "src.process.name", "contains", second_list, None),
    ]
...
def test_get_query_text_handles_same_field_different_tag_dv(s1_product: SentinelOne):
    ...
    assert s1_product._get_query_text() == [
        (Tag("valueA", data=None), 'ProcessName containscis "svchost.exe"'),
        (Tag("valueB", data=None), 'ProcessName containscis "cmd.exe"'),
    ]


def test_get_query_text_handles_different_fields_different_tag_dv(
    s1_product: SentinelOne,
):
    sdate = datetime.now(timezone.utc)
    edate = sdate - timedelta(days=7)
    ...
    s1_product._queries = {
        Tag("valueA"): [
            Query(sdate, edate, "ProcessName", "containscis", '"posh.exe"')
        ],
    ...
    s1_product._queries = {
        Tag("valueA"): [
            Query(
                ...
                None,
                None,
                'src.process.name contains "explorer.exe"',
            )
        ]
    }
...
def test_process_queries_dv(s1_product: SentinelOne, mocker):
    ...
    s1_product._queries = {
        Tag("valueA", data=None): [
            Query(sdate, edate, "ProcessName", "containscis", "powershell.exe"),
        ...
        Tag("valueB", data=None): [
            Query(sdate, edate, "DNS", "containscis", "google.com"),
    ...
    mocked_run_query.assert_has_calls(
        [
            call(
                "ProcessName containscis powershell.exe OR ProcessName containscis cmd.exe OR ProcessName "
            ...
            call(
                "ProcessName containscis iexplore.exe",
                ANY,
                ANY,
                Tag("valueA", data=None),
                ANY,
            ...
            call(
                "DNS containscis google.com OR DNS containscis microsoft.com OR DNS containscis amazon.com OR DNS containscis bing.com "
...
def test_process_queries_pq(s1_product: SentinelOne, mocker):
    ...
    s1_product._queries = {
        Tag("valueA", data=None): [
            Query(sdate, edate, "src.process.name", "contains", "powershell.exe"),
        ...
        Tag("valueB", data=None): [
            Query(sdate, edate, "event.dns.request", "contains", "google.com"),
    ...
    mocked_run_query.assert_has_calls(
        [
            call(
                "src.process.name contains powershell.exe OR src.process.name contains cmd.exe OR src.process.name contains "
            ...
            call(
                "src.process.name contains iexplore.exe | group count() by endpoint.name, src.process.user, "
                + "src.process.image.path, src.process.cmdline, event.time, "
                + "site.id, site.name, src.process.storyline.id, src.process.displayname, "
                + "src.process.parent.image.path, tgt.process.displayname, tgt.process.image.path, "
                + "tgt.file.path, tgt.file.sha1, tgt.file.sha256, url.address, src.ip.address, "
            ...
            call(
                "event.dns.request contains google.com OR event.dns.request contains microsoft.com OR event.dns.request contains amazon.com OR event.dns.request contains bing.com "
            ...
            call(
                "event.dns.request contains apple.com | group count() by endpoint.name, src.process.user, "
                + "src.process.image.path, src.process.cmdline, event.time, "
                + "site.id, site.name, src.process.storyline.id, src.process.displayname, "
                + "src.process.parent.image.path, tgt.process.displayname, tgt.process.image.path, "
                + "tgt.file.path, tgt.file.sha1, tgt.file.sha256, url.address, src.ip.address, "
...
def test_process_queries_pq_single_site_id(s1_product: SentinelOne, mocker):
    ...
    mocked_run_query.assert_has_calls(
        [
            call(
                "(src.process.name contains powershell.exe) AND (site.id = 12345) "
                + "| group count() by endpoint.name, src.process.user, "
                + "src.process.image.path, src.process.cmdline, event.time, "
                + "site.id, site.name, src.process.storyline.id, src.process.displayname, "
                + "src.process.parent.image.path, tgt.process.displayname, tgt.process.image.path, "
...
def test_process_queries_pq_multiple_site_ids(s1_product: SentinelOne, mocker):
    ...
    mocked_run_query.assert_has_calls(
        [
            call(
                "(src.process.name contains powershell.exe) AND (site.id = 12345 OR site.id = 67890) "
                + "| group count() by endpoint.name, src.process.user, "
                + "src.process.image.path, src.process.cmdline, event.time, "
                + "site.id, site.name, src.process.storyline.id, src.process.displayname, "
                + "src.process.parent.image.path, tgt.process.displayname, tgt.process.image.path, "
