...
PARAMETER_MAPPING_DV: dict[str, list[str]] = {
    "query": [
        "query"
    ],  # non-existent field to specify a fully defined query string in a definition file.
...
class SentinelOne(Product):
    ...
    def __init__(self, pq: bool = False, **kwargs):
        ...
        if "deep_visibility" in kwargs:
            self._pq = (
                False if kwargs.get("deep_visibility", "False") == "True" else True
            )
    ...
    def _authenticate(self):
        if self._url and self._token:
            ...
            if self.profile and self.profile not in config:
                raise ValueError(
                    f"Profile {self.profile} is not present in credential file or no profile has been provided. Please validate profile or ensure profile is provided."
                )
            ...
            if "url" not in section:
                raise ValueError('S1 configuration invalid, ensure "url" is specified')
            ...
            if "token" in section:
                ...
                if "S1_TOKEN" not in os.environ:
                    raise ValueError(
                        'S1 configuration invalid, specify "token" configuration value or "S1_TOKEN" '
        ...
        self._session.mount(
            "https://", HTTPAdapter(pool_connections=10, pool_maxsize=10, max_retries=3)
        )
        ...
        if len(self._site_ids) < 1 and len(self._account_ids) < 1:
            raise ValueError(
                "S1 configuration invalid, specify a site_id, account_id, or account_name"
            )
                ...
                if counter == 10 or i == len(account_ids) - 1:
                    try:
                        response = self._get_all_paginated_data(
                            self._build_url("/web/api/v2.1/accounts"),
                        ...
                        if e.response.status_code == 401:
                            raise AuthenticationError(
                                "Failed to authenticate to SentinelOne API"
                            ) from e
                        raise
        ...
        if site_ids:
            ...
            while i < len(site_ids):
                ...
                if counter == 10 or i == len(site_ids) - 1:
                    try:
                        response = self._get_all_paginated_data(
                            self._build_url("/web/api/v2.1/sites"),
                    ...
                    for item in response:
                        for site in item["sites"]:
                            ...
                            if self._pq:
                                ...
                                site["accountId"] not in self._account_ids
                                and site["id"] not in self._site_ids
                            ):
                                self._site_ids.append(site["id"])
    ...
    def build_query(self, filters: dict) -> Tuple[str, datetime, datetime]:
        ...
        for key, value in filters.items():
            if key == "days":
                ...
                self._echo(
                    f"Query filter {key} is not supported by product {self.product}",
                    logging.WARNING,
                )
    ...
    def _get_all_paginated_data(
        ...
        progress_desc: str = "Retrieving data",
        add_default_params: bool = True,
    ) -> list[dict]:
        """
        ...
        next_cursor = True
        with tqdm(
            desc=progress_desc, disable=not self._tqdm_echo or no_progress
        ) as p_bar:
            while next_cursor:
    ...
    def _get_dv_events(
        self, query_id: str, cancel_event: Event, p_bar_needed: bool = True
    ) -> list[dict]:
        """
        ...
        def errors(_response_data: dict[str, Any]):
            return (
                _response_data["errors"]
                if self._pq
                else _response_data["data"]["responseError"]
            )

        def current_progress(_response_data: dict[str, Any]) -> int:
            return (
                _response_data["data"]["progress"]
                if self._pq
                else _response_data["data"]["progressStatus"]
            )

        def current_status(_response_data: dict[str, Any]) -> int:
            return (
                _response_data["data"]["status"]
                if self._pq
                else _response_data["data"]["responseState"]
            )
        ...
        try:
            ...
            while not cancel_event.is_set():
                url = (
                    "/web/api/v2.1/dv/events/pq-ping"
                    if self._pq
                    else "/web/api/v2.1/dv/query-status"
                )
                query_status_response = self._session.get(
                    self._build_url(url),
                ...
                if progress == 100 or status == "FAILED":
                    if status == "FAILED":
                        raise ValueError(
                            f'S1 query failed with message "{errors(response_data)}"'
                        )
                    ...
                    if self._pq:
                        ...
                        return self._get_all_paginated_data(
                            self._build_url("/web/api/v2.1/dv/events"),
                            params={"queryId": query_id},
                            no_progress=False,
                            add_default_params=False,
    ...
    def divide_chunks(self, list_input: list, n: int):
        for i in range(0, len(list_input), n):
            yield list_input[i : i + n]
        ...
        try:
            for search_field, terms in criteria.items():
                if search_field not in self.parameter_mapping:
                    self._echo(
                        f"Query filter {search_field} is not supported by product {self.product}",
                        logging.WARNING,
                    )
                    continue
                ...
                if self._pq:
                    for param in parameter:
                        if param == "query":
                            ...
                            self._queries[tag].append(
                                Query(
                                    from_date, to_date, None, None, None, search_value
                                )
                            )
                        elif (
                            (sum(len(i) for i in terms) + 300) / 8192 >= 0.75
                        ):  # chunk terms if query is suspected to contain more than 8192 total characters (current PQ limitation)
                            char_num = (
                                int((sum(len(i) for i in terms)) / 6144) + 1
                            )  # divide total characters of terms by 75% of limit to identify chunk number
                            chunk_quantity = int(
                                len(terms) / char_num
                            )  # determine number of terms per chunk to evenly split list of terms
                            chunked_terms = list(
                                self.divide_chunks(terms, chunk_quantity)
                            )
                            ...
                            self._queries[tag].append(
                                Query(
                                    from_date, to_date, param, "contains", search_value
                                )
                            )
                else:
                    ...
                    for chunk in chunked_terms:
                        ...
                        for param in parameter:
                            ...
                            if param == "query":
                                ...
                                r"\w+\.\w+", search_value
                            ) and tag.tag.startswith("IOC - "):
                                operator = "regexp"
                            else:
                                operator = "containscis"

                            self._queries[tag].append(
                                Query(from_date, to_date, param, operator, search_value)
                            )
            ...
            for tag, queries in self._queries.items():
                for query in queries:
                    if query.full_query is not None:
                        ...
                        full_query = (
                            f"{query.parameter} {query.operator} {query.search_value}"
                        )
    ...
    def _run_query(
        ...
        try:
                ...
                params.update(
                    {
                        ...
                        "queryType": [
                            "events"
                        ],  # options: 'events', 'procesState' (deprecated)
                    }
                )
            ...
            url = (
                "/web/api/v2.1/dv/events/pq"
                if self._pq
                else "/web/api/v2.1/dv/init-query"
            )
            ...
            if "errors" in body and any(
                ("could not parse query" in x["detail"] for x in body["errors"])
            ):
            ...
            if (
                self._pq and body["data"]["status"] == "FINISHED"
            ):  # If using PQ, the results can be returned immediately
                events = body["data"]["data"]
            else:
                events = self._get_dv_events(
                    query_id, p_bar_needed=p_bar_needed, cancel_event=cancel_event
                )
            ...
            for event in events:
                if self._pq:
                    ...
                    srcprocstorylineid = (
                        event["srcProcStorylineId"]
                        if "srcProcStorylineId" in event
                        else "None"
                    )
                    srcprocdisplayname = (
                        event["srcProcDisplayName"]
                        if "srcProcDisplayName" in event
                        else "None"
                    )
                    tgtprocdisplayname = (
                        event["tgtProcDisplayName"]
                        if "tgtProcDisplayName" in event
                        else "None"
                    )
                    tgtfilepath = (
                        event["tgtFilePath"] if "tgtFilePath" in event else "None"
                    )
                    tgtfilesha1 = event["fileSha1"] if "fileSha1" in event else "None"
                    tgtfilesha256 = (
                        event["fileSha256"] if "fileSha256" in event else "None"
                    )
                    scrprocparentimagepath = (
                        event["srcProcParentImagePath"]
                        if "srcProcParentImagePath" in event
                        else "None"
                    )
                    tgtprocimagepath = (
                        event["tgtProcImagePath"]
                        if "tgtProcImagePath" in event
                        else "None"
                    )
                    ...
                    dnsrequest = (
                        event["dnsRequest"] if "dnsRequest" in event else "None"
                    )
    ...
    def _process_queries(self) -> None:
        ...
        chunk_size = (
            1 if (len(ioc_hunt) == 1 and ioc_hunt[0].tag.startswith("IOC - ")) else 10
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=25) as executor:
            ...
            p_bar = tqdm(
                desc="Running queries", disable=not self._tqdm_echo, total=len(futures)
            )

            try:
                completed_futures = set[Future]()
                while not cancel_event.is_set() and len(completed_futures) != len(
                    futures
                ):
    ...
    def get_results(self, final_call: bool = True) -> dict[Tag, list[Result]]:
        ...
        if final_call and len(self._queries) > 0:
            self.log.debug("Executing additional _process_queries")
