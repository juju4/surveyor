...
def _convert_relative_time(relative_time) -> str:
    ...
    device_timestamp = "device_timestamp:[{0} TO {1}]".format(
...
class CbEnterpriseEdr(Product):
    ...
    def __init__(self, **kwargs):
        ...
        self._device_group = (
            kwargs["device_group"] if "device_group" in kwargs else None
        )
        self._device_policy = (
            kwargs["device_policy"] if "device_group" in kwargs else None
        )
    ...
    def divide_chunks(self, list_input: list, n: int) -> Generator:
    ...
    def perform_query(self, tag: Tag, base_query: dict, query: str) -> set[Result]:
        ...
        try:
            ...
            for proc in process.where(full_query):
                ...
                user = (
                    deets["process_username"][0]
                    if "process_username" in deets
                    else "None"
                )
                proc_name = deets["process_name"] if "process_name" in deets else "None"
                cmdline = (
                    deets["process_cmdline"][0]
                    if "process_cmdline" in deets
                    else "None"
                )
                ts = (
                    deets["device_timestamp"] if "device_timestamp" in deets else "None"
                )
            ...
            if search_field == "query":
                ...
                for chunk in chunked_terms:
                    ...
                    query = (
                        "("
                        + " OR ".join(
                            "%s:%s" % (PARAMETER_MAPPING[search_field], term)
                            for term in terms
                        )
                        + ")"
                    )
