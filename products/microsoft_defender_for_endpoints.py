...
PARAMETER_MAPPING: dict[str, dict[str, Union[str, list[str]]]] = {
    ...
    "internal_name": {
        "table": "DeviceProcessEvents",
        "field": "ProcessVersionInfoInternalFileName",
        "projections": [
            "DeviceName",
            "AccountName",
            "FolderPath",
...
class DefenderForEndpoints(Product):
    ...
    def _authenticate(self) -> None:
        if not self._token:
            if self._tenantId and self._appId and self._appSecret:
                self._token = self._get_aad_token(
                    self._tenantId, self._appId, self._appSecret
                )
                ...
                if self.profile not in config:
                    raise ValueError(
                        f"Profile {self.profile} is not present in credential file"
                    )
                ...
                if "token" in section:
                    ...
                    "tenantId" not in section
                    or "appId" not in section
                    or "appSecret" not in section
                ):
                    raise ValueError(
                        "Credential file must contain a token or the fields tenantId, appId, and appSecret values"
                    )
                else:
                    self._token = self._get_aad_token(
                        section["tenantId"], section["appId"], section["appSecret"]
                    )
    ...
    def _get_aad_token(self, tenant_id: str, app_id: str, app_secret: str) -> str:
        ...
        """
        self.log.debug(
            f"Acquiring AAD access token for tenant {tenant_id} and app {app_id}"
        )
                ...
                for res in response.json()["Results"]:
                    ...
                    if "AccountName" in res or "InitiatingProcessAccountName" in res:
                        username = (
                            ...
                            else res["InitiatingProcessAccountName"]
                        )
                    else:
                        username = "Unknown"

                    if (
                        "ProcessCommandLine" in res
                        or "InitiatingProcessCommandLine" in res
                    ):
                        cmdline = (
                            res["ProcessCommandLine"]
                            if "ProcessCommandLine" in res
                            else res["InitiatingProcessCommandLine"]
                        )
                    else:
                        cmdline = "Unknown"
                    ...
                    if "FolderPath" in res or "InitiatingProcessFolderPath" in res:
                        proc_name = (
                            res["FolderPath"]
                            if "FolderPath" in res
                            else res["InitiatingProcessFolderPath"]
                        )
                    else:
                        proc_name = "Unknown"
                    ...
                    result = Result(
                        hostname, username, proc_name, cmdline, (timestamp,)
                    )
                    results.add(result)
            else:
                self._echo(
                    f"Received status code: {response.status_code} (message: {response.json()})"
                )
    ...
    def process_search(self, tag: Tag, base_query: dict, query: str) -> None:
        ...
        results = self._post_advanced_query(
            data=full_query, headers=self._get_default_header()
        )
        ...
        try:
            for search_field, terms in criteria.items():
                if search_field == "query":
                    ...
                    query += (
                        f"{(PARAMETER_MAPPING[search_field]['additional'])} "
                        if "additional" in PARAMETER_MAPPING[search_field]
                        else ""
                    )
