...
class CortexXDR(Product):
    ...
    _auth_type: Optional[str] = (
        "standard"  # Either standard or advanced, default is standard
    )
    ...
    _limit: int = (
        1000  # Max is 1000 results otherwise have to get the results via stream
    )
    _raw: bool = False
    ...
    def _authenticate(self) -> None:
        if not (self._url and self._api_key and self._api_key_id and self._auth_type):
            if not os.path.isfile(self.creds_file):
                ...
                if self.profile not in config or not self.profile:
                    raise ValueError(
                        f"Profile {self.profile} is not present in credential file or a profile argument was not passed. Please retry"
                    )
                ...
                if "url" not in section:
                    raise ValueError(
                        'Cortex XDR configuration invalid, ensure "url" is specified'
                    )
                ...
                if "api_key" in section:
                    ...
                    raise ValueError(
                        'Cortex XDR configuration invalid, ensure "api_key" is specified'
                    )
                ...
                if "api_key_id" in section:
                    ...
                    raise ValueError(
                        'Cortex XDR configuration invalid, ensure "api_key_id" is specified'
                    )
                ...
                if "auth_type" in section:
                    if section["auth_type"].lower() in ["standard", "advanced"]:
                        ...
                        raise ValueError(
                            'Cortex XDR configuration invalid, ensure "auth_type" is one of ["standard","advanced"]'
                        )
        ...
        auth_check = self._session.post(
            self._build_url("/public_api/v1/xql/get_quota"),
        ...
        if "reply" not in resp:
            raise ValueError(
                "Cortex could not validate credentials. Please check your configuration file and try again."
            )
        else:
    ...
    def _get_default_header(self) -> dict:
        ...
        if self._auth_type == "advanced":
            ...
            return {
                ...
                "x-xdr-auth-id": f"{self._api_key_id}",
            ...
            for search_field, terms in criteria.items():
                ...
                if search_field == "query":
                        ...
                        if len(terms) > 1:
                            for term in terms:
                                self._queries[tag].append(
                                    Query(relative_time_ms, None, None, None, term)
                                )
                        else:
                            self._queries[tag].append(
                                Query(relative_time_ms, None, None, None, terms[0])
                            )
                    else:
                        self._queries[tag].append(
                            Query(relative_time_ms, None, None, None, terms)
                        )
                else:
                    ...
                    if len(terms) > 1:
                        ...
                        all_terms = ", ".join(
                            (f'"*{term}*"').replace("**", "*") for term in terms
                        )
                    ...
                    self._queries[tag].append(
                        Query(relative_time_ms, parameter, operator, search_value)
                    )
    ...
    def _get_xql_results(self, query_id: str) -> Tuple[dict, int]:
        ...
        try:
            while True:
                query_status_response = self._session.post(
                    self._build_url("/public_api/v1/xql/get_query_results"),
                    headers=self._get_default_header(),
                    json=params,
                )
    ...
    def _process_queries(self) -> None:
        for tag, queries in self._queries.items():
            for query in queries:
                ...
                query_string += " | fields agent_hostname, action_process_image_path, action_process_username, action_process_image_command_line, actor_process_image_path, actor_primary_username, actor_process_command_line, event_id"
                ...
                query_response = self._session.post(
                    self._build_url("/public_api/v1/xql/start_xql_query/"),
                    headers=self._get_default_header(),
                ...
                if "reply" not in body:
                    raise ValueError(
                        f'Cortex encountered an error and could not process query "{query_string}"'
                    )
                ...
                if count == 1000:
                    self.log.info("Maximum limit of results (1000) was reached")
                else:
                ...
                for event in events:
                    hostname = (
                        event["agent_hostname"] if "agent_hostname" in event else ""
                    )

                    # If the event is not a process execution, we need to see what process initiated the filemod, regmod, netconn, etc.
                    username = (
                        event["action_process_username"]
                        if "action_process_username" in event
                        else event["actor_primary_username"]
                    )
                    path = (
                        event["action_process_image_path"]
                        if "action_process_image_path" in event
                        else event["actor_process_image_path"]
                    )
                    commandline = (
                        event["action_process_command_line"]
                        if "action_process_command_line" in event
                        else event["actor_process_command_line"]
                    )
                    ...
                    """
                    result = Result(
                        hostname, username, path, commandline, additional_data
                    )
                    self._results[tag].append(result)
