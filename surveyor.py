...
if sys.version_info.major == 3 and sys.version_info.minor < 9:
    print(
        f"Python 3.9+ is required to run Surveyor (current: {sys.version_info.major}.{sys.version_info.minor})"
    )
    exit(1)
...
def _write_results(
    ...
    if output:
        ...
        if len(results) > 0:
            log_echo(
                f"\033[92m-->{tag.tag}: {len(results)} results \033[0m",
                log,
                use_tqdm=use_tqdm,
            )
        else:
...
@click.option(
    "--sigmarule",
    "sigma_rule",
    help="Sigma rule file to process (must be in YAML format).",
    type=click.STRING,
)
@click.option(
    "--sigmadir",
    "sigma_dir",
    help="Directory containing multiple sigma rule files.",
    type=click.STRING,
)
# optional output
@click.option(
...
@click.option(
    "--no-file",
    help="Write results to STDOUT instead of the output CSV",
    is_flag=True,
    default=False,
)
@click.option(
...
@click.option(
    "--site-id", help="ID of SentinelOne site to query", multiple=True, default=None
)
@click.option(
    "--account-id",
    help="ID of SentinelOne account to query",
    multiple=True,
    default=None,
)
@click.option(
    "--account-name",
    help="Name of SentinelOne account to query",
    multiple=True,
    default=None,
)
@click.option(
...
@click.option(
    "--dv", "dv", help="Use Deep Visibility for queries", is_flag=True, required=False
)
@click.pass_context
def s1(
    ctx,
    site_id: Optional[Tuple],
    account_id: Optional[Tuple],
    account_name: Optional[Tuple],
    creds: Optional[str],
    dv: bool,
) -> None:
...
@click.option(
    "--device-group", help="Name of device group to query", multiple=True, default=None
)
@click.option(
    "--device-policy",
    help="Name of device policy to query",
    multiple=True,
    default=None,
)
@click.pass_context
...
@click.option(
    "--sensor-group", help="Name of sensor group to query", multiple=True, default=None
)
@click.pass_context
...
def survey(ctx, product_str: str = "cbr") -> None:
    ...
    log_file_name = (
        datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S") + f".{product_str}.log"
    )
    ...
    if not opt.no_file:
        ...
        template_str = (
            f"{{:<{table_template[0]}}} {{:<{table_template[1]}}} {{:<{table_template[2]}}} "
            f"{{:<{table_template[3]}}}"
        )
    ...
    try:
        ...
        if opt.def_file:
            if not os.path.exists(opt.def_file):
                repo_deffile: str = os.path.join(
                    os.path.dirname(__file__), "definitions", opt.def_file
                )
        ...
        if opt.ioc_file:
            with open(opt.ioc_file) as ioc_file:
                ...
                product.nested_process_search(
                    Tag(f"IOC - {opt.ioc_file}", data=basename),
                    {opt.ioc_type: ioc_list},
                    base_query,
                )
        ...
        if opt.def_file is not None or opt.def_dir is not None:
            for definitions in tqdm(
                definition_files,
                desc="Processing definition files",
                ...
                with open(definitions, "r") as file:
                    ...
                    for program, criteria in programs.items():
                        product.nested_process_search(
                            Tag(program, data=source), criteria, base_query
                        )
        ...
        if len(sigma_rules) > 0:
            pq_check = (
                True if "pq" in opt.product_args and opt.product_args["pq"] else False
            )
            translated_rules = sigma_translation(product_str, sigma_rules, pq_check)
            if len(translated_rules["queries"]) != len(sigma_rules):
                log.warning(
                    f"Only {len(translated_rules['queries'])} out of {len(sigma_rules)} were able to be translated."
                )
            for rule in tqdm(
                translated_rules["queries"],
                desc="Processing sigma rules",
                disable=opt.no_progress,
            ):
                ...
                product.nested_process_search(
                    Tag(program, data=source), {"query": [rule["query"]]}, base_query
                )
        ...
        log_echo(
            f"Caught {type(e).__name__} (see log for details): {e}", log, logging.ERROR
        )
        log.exception(e)
    finally:
        if output_file:
...
for product_name in get_products():
    ...
    if product_name not in dir_res:
        cli.command(name=product_name, help=f"Query {product_name}")(
            create_generic_product_command(str(product_name))
        )
